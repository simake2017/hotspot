...
LoadJavaVM(const char *jvmpath, InvocationFunctions *ifn)
{
    ...
    /*
     * 调用 相应的 jni函数
     * 因为不同的 虚拟机环境下 调用 的 JNI_CreateJavaVM 是不同的
     */
    ifn->CreateJavaVM = (CreateJavaVM_t)
      dlsym(libjvm, "JNI_CreateJavaVM");    //wangyang 这里调用 jni_createJavaVm --> 这里不是 进行调用，而是对 这个函数指针进行赋值
    if (ifn->CreateJavaVM == NULL) //这里只是判断 是否为空，并不是真的执行该函数
        goto error;
