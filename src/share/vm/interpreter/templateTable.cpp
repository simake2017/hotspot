...
/*
 * 用InterpreterGenerater的 _masm初始化 当前 的masm
 *
 */
...
void TemplateTable::def(Bytecodes::Code code, int flags, TosState in, TosState out, void (*gen)(int arg), int arg) {
  ...
  /*
   * 使用下面  template_for 找到对应的 code的模板
   *
   */
   ...
   *
   */
  t->initialize(flags, in, out, gen, arg); //通过指针调用静态方法 进行 初始化， 这里相当于 Template::initialize ，纠结了半天，没地方初始化 数组
