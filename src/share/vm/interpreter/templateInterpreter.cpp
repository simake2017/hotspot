...
void TemplateInterpreter::initialize() {
  ...
  /*
   * wangyang
   * 对指令集初始化，初始化到这个 TemplateeTable中
   */
  TemplateTable::initialize(); // 模板信息初始化 ， 将对应的 字节码指令 初始化到 template_array数组中
  ...
  { ResourceMark rm;
     ...
     * 在这里 初始化
     * 这里对 模板 解释器进行初始化
     * 这里是  例程的初始化 ，方法 的进入 异常处理 等等
     */
    InterpreterGenerator g(_code); // stubQueue的地址 ，这里是命名的方式去调用
...
#undef method_entry
  ...
  // Bytecodes
  set_entry_points_for_all_bytes(); // 生成  字节码 指令 ------------------------
...
/*
 * wangyang
 * 这里为每个 字节码指令生成 一个 CodeletMark
 *
 *
 */

void TemplateInterpreterGenerator:: (Bytecodes::Code code) {
  CodeletMark cm(_masm, Bytecodes::name(code), code); //这里再次 初始化 _masm
...
void TemplateInterpreterGenerator::generate_and_dispatch(Template* t, TosState tos_out) {
  ...
  // generate template
  t->generate(_masm); //初始化 模板的 模板解释器
  // advance
  if (t->does_dispatch()) {
#ifdef ASSERT
