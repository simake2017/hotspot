...
BufferBlob* BufferBlob::create(const char* name, int buffer_size) {
  ...
  assert(name != NULL, "must provide a name");
  {
    ...
    /*
     * 这是一种放置语法，在size的位置，开始创建对象
     * 这里的size 是内存中的 一个位置
     * wangyang 上面的解释有问题
     *
     * 这里的new是一个构造函数 类里面进行了 重写，可以点进去
     */
...
/**
 * wangyang
 * 这里 使用new 运算符进行了  operator 重写 了 new运算符
 * 这里重写的目的是在heap中进行分配，否则调用c++默认提供的函数，会在 进程堆中分配
 */
