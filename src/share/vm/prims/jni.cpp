...
JNI_ENTRY(jclass, jni_FindClass(JNIEnv *env, const char *name))
  ...
  if (k.not_null()) {
    ...
    loader = Handle(THREAD, SystemDictionary::java_system_loader()); ///////---------> 执行 这条 ，这里的handle是 system_class_loader
  }
...
/*
 *这里是一个宏 ，最终 声明 出 一段代码 以 jni_end结束
 *下面 的 宏 会声明 一个 函数
 *
...
/*
 * 这里是对 jni.h中声明的 JNINativeInterface_ 接口内容的声明
 *
 */
struct JNINativeInterface_ jni_NativeInterface = {
	/*

	开始4个 null 声明了 这4个 变量
	void *reserved0;
    void *reserved1;
    void *reserved2;
 ...
 * 创建java执行环境
 * 最终执行下面的 jvm 函数
 * 这里第二个参数  penv 就是 env 环境 变量
 *
 * 这里是 在java.c 里面进行调用的，并且这里的vm penv初值都是0 表示 一个初始值
 *
 */
_JNI_IMPORT_OR_EXPORT_ jint JNICALL JNI_CreateJavaVM(JavaVM **vm, void **penv, void *args) {
  ...
  if (result == JNI_OK) {
    JavaThread *thread = JavaThread::current(); //获取当前Java线程，
    /* thread is thread_in_vm here */
    *vm = (JavaVM *)(&main_vm); //--> 这里会进行一个赋值 操作
    *(JNIEnv**)penv = thread->jni_environment(); //wangyang important 赋值为 当前线程的  jni_environment
