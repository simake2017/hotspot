...
/*
 * wangyang
 * 这里存储的是  已知的 KlassOop类型
 * 这里都是预先加载的 类 的 klass 对象
 *
 * 这里存储的都是
 *
 */
...
/*
 wk_klass (ClassLoader_klass) --> template(ClassLoader_klass,            java_lang_ClassLoader,
 *返回 java/lang/classLoader 对应的 klassOop
 */
void SystemDictionary::compute_java_system_loader(TRAPS) {
   ...
   * 调用 java 方法 (静态的)
   * 调用 classLoader 的
   * 获取对应 Klass 对象 通过  方法名称和方法签名 进行调用
   */
  JavaCalls::call_static(&result, //--> result用于承载 返回值
                         KlassHandle(THREAD, WK_KLASS(ClassLoader_klass)), //--> 用于 获取 方法所在的类 ，这里 是 ClassLoader类
                         vmSymbols::getSystemClassLoader_name(),  //--> 方法名称
                         vmSymbols::void_classloader_signature(), //--> 方法签名
                         CHECK);

  _java_system_loader = (oop)result.get_jobject(); //返回 result 的返回 对象 转为 oop类型
}
...
klassOop SystemDictionary::resolve_or_fail(Symbol* class_name, Handle class_loader, Handle protection_domain, bool throw_error, TRAPS) {
  klassOop klass = resolve_or_null(class_name, class_loader, protection_domain, THREAD);///////--> 这条
...
klassOop SystemDictionary::resolve_instance_class_or_null(Symbol* name, Handle class_loader, Handle protection_domain, TRAPS) {
  ...
  if (!class_has_been_loaded) {
    ...
    if (!class_has_been_loaded) {
      ...
      // Do actual loading
      k = load_instance_class(name, class_loader, THREAD);  //_------------------------- 这里
      ...
      if (!HAS_PENDING_EXCEPTION && !k.is_null() &&
        ...
        if (!HAS_PENDING_EXCEPTION) {
          { // Grabbing the Compile_lock prevents systemDictionary updates
            ...
            /*
             * wangyang yyyyyyyyyyyyy
             * 这里重要，这里会把加载到的类 更新到 dictionary中
             */
...
klassOop SystemDictionary::resolve_from_stream(Symbol* class_name,
  ...
  /*
   * 这里真正调用  parseClassFile
   *  将 流 转为 classFile ，然后 转为 Klass
   */
  instanceKlassHandle k = ClassFileParser(st).parseClassFile(class_name,
                                                             class_loader,
                                                             protection_domain,
  ...
  if (class_loader.is_null()) {
    ...
    if (MustCallLoadClassInternal && has_loadClassInternal()) {
      JavaCalls::call_special(&result,
    	 ...
    	 * wangyang
    	 * 这里解释一下，
    	 * 这里调用 的class_loader是 system classloader
    	 * 调用loadClass_name 这个方法，
    	 * string_class_signature 方法签名是他
    	 */
...
void SystemDictionary::initialize(TRAPS) {
   ...
   * 加载已知的 klass
   */
  initialize_preloaded_classes(CHECK);  //--> 这里
}

// Compact table of directions on the initialization of klasses:
/*

 比如 vmSymbols::java_lang_Object_enum 是 1 左移 4位
...
bool SystemDictionary::initialize_wk_klass(WKID id, int init_opt, TRAPS) {
  ...
  int  sid  = (info >> CEIL_LG_OPTION_LIMIT); //wangyang --> 通过右移 在移过去 获取sid
  Symbol* symbol = vmSymbols::symbol_at((vmSymbols::SID)sid); //--> 获取对应的名称
  klassOop*    klassp = &_well_known_klasses[id]; //这里是对应的 klassOop
  ...
  /*
   * wangyang
   * 调用下面的方法 解析一个 klass 从 jar包中
   * 这里是让 这个 数组 对应位置的地址  指向一个 klassOop
   */
  if ((*klassp) == NULL && try_load) {
    if (must_load) { //这里是必须加载
