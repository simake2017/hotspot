...
/**
 * wangyang
 * system directionary 存储所有的  class名称 和 classLoader的对应关系
 */
...
/**
 *
 * 这里  tempalte 不能当做 关键字处理  tempalte 就是类似一个 普通的 模板 中的符号
 * wangyang
 * 重要
 */
...
class SystemDictionary : AllStatic {
 ...
 public:
  enum WKID {
    ...
    FIRST_WKID = NO_WKID + 1 // --> 这里是 第一个 wkid
  };
  ...
  enum InitOption {
    ...
    OPTION_LIMIT,
    CEIL_LG_OPTION_LIMIT = 4    // OPTION_LIMIT <= (1<<CEIL_LG_OPTION_LIMIT) //--> 左移 4位
  };
...
public:
  ...
  /*
   * wangyang
   * _well_known_klasses 是 已加载的
   * static klassOop Class_klass() { return check_klass_Pre(_well_known_klasses[Class_klass_knum]); }

  static klassOop Object_klass() { return check_klass_Pre(_well_known_klasses[Object_klass_knum]); } \
  static klassOop String_klass() { return check_klass_Pre(_well_known_klasses[String_klass_knum]); } \
  static klassOop Class_klass() { return check_klass_Pre(_well_known_klasses[Class_klass_knum]); } \
  static klassOop Cloneable_klass() { return check_klass_Pre(_well_known_klasses[Cloneable_klass_knum]); } \
  static klassOop ClassLoader_klass() { return check_klass_Pre(_well_known_klasses[ClassLoader_klass_knum]); } \
  static klassOop Serializable_klass() { return check_klass_Pre(_well_known_klasses[Serializable_klass_knum]); } \
  static klassOop System_klass() { return check_klass_Pre(_well_known_klasses[System_klass_knum]); } \
  static klassOop Throwable_klass() { return check_klass_Pre(_well_known_klasses[Throwable_klass_knum]); } \


   */

  /**
   * @brief Construct a new wk klasses do object
   * 通过模板的形式 生成 相应的 方法，如上面所示
   * 
   */
  ...
  /**
   * @brief 
   * 这里在 获取的时候 会获取相应的 类的 Klass 对象 ，通过一个name 来获取      
   * 
   */
