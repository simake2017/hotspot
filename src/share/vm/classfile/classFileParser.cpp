...
constantPoolHandle ClassFileParser::parse_constant_pool(TRAPS) {
  ...
  u2 length = cfs->get_u2_fast(); //通过 class 文件 获取的 长度
  guarantee_property(
...
typeArrayHandle ClassFileParser::parse_fields(Symbol* class_name,
  ...
  u2 length = cfs->get_u2_fast(); //wangyang 用于获取 字段数量
  *java_fields_count_ptr = length;
  ...
  /*
   * wangyang
   * length 是 u2类型 占2个字节，所以 一共使用 u2 * 7 = 14 个 字节 ，及 在klass类中，一个类占用14个 字节 大小
   *
   */
  ...
  for (int n = 0; n < length; n++) {
     ...
     * wangyang
     * 读取类型索引， 同样指向常量池的一项索引
     * 比如 java/lang/String 类型 java/lang/Integer类型
     */
    u2 signature_index = cfs->get_u2_fast();
    check_property(
  ...
  if (num_injected != 0) {
    ...
    if (index < length + num_injected) {
      ...
      for (int i = 0; i < index * FieldInfo::field_slots; i++) {
        new_fields->short_at_put(i, fields->short_at(i));  //--> 用于 存放 对应的 字段
      }
      fields = new_fields;
    }
  }
...
instanceKlassHandle ClassFileParser::parseClassFile(Symbol* name,
  ...
  { HandleMark hm(THREAD);
    ...
    // wangyang 计算起始偏移量  1.6 静态字段 保存在  mirror 镜像类 中  起始位置就是 mirro类实例的结束位置
    ...
    /*
     * wangyang
     * 这里用于计算5大类型的 静态 类型 的偏移量
     *
     */
    next_static_word_offset     = next_static_double_offset +
                                  (fac.count[STATIC_DOUBLE] * BytesPerLong);  // 计算 对应 类型的 字段数量
     ...
     * wangyang
     * 计算 非静态字段 偏移量
     * 第一个非静态字段偏移量 ，要么是按照 4 对齐 要么是 按照 8对齐     比如子类 对于父类 也是  按照 这个 进行偏移
     * nonstatic_field_size  --> 需要判断是否有父类 父类的 非静态字段数量 * heapOopSize
     */
    ...
    if( nonstatic_double_count > 0 ) {
      ...
      next_nonstatic_double_offset = align_size_up(offset, BytesPerLong);  //内存对齐  找到 真正的位置
    ...
    next_nonstatic_short_offset = next_nonstatic_word_offset +
                                  (nonstatic_word_count * BytesPerInt);
    next_nonstatic_byte_offset  = next_nonstatic_short_offset +
                                  (nonstatic_short_count * BytesPerShort); // 一字节字段对齐
    ...
    if( allocation_style == 0 ) {
      notaligned_offset = next_nonstatic_byte_offset + nonstatic_byte_count; //非对齐
