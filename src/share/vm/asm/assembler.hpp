...
class AbstractAssembler : public ResourceObj  {
 ...
 protected:
  ...
  address      _code_begin;            // first byte of code buffer 指令缓存区 的首地址
  address      _code_limit;            // first byte after code buffer
  address      _code_pos;              // current code generation position 指令缓存区的当前位置
  ...
  void emit_byte(int x);  // emit a single byte  往指令区写入一个 字节数据 指令
  void emit_word(int x);  // emit a 16-bit word (not a wordSize word!) // 写入一个 16位 数据
  void emit_long(jint x); // emit a 32-bit word (not a longSize word!) // 写入一个 32 位 数据
  void emit_address(address x); // emit an address (not a longSize word!) // 写入 一个 地址 数据
