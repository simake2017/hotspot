...
/**
 * wangyang
 * Threads类 连接 所有激活线程，并且提供操作供所有线程使用，操作都是被
 * 独占锁 保护
 */
...
jint Threads::create_vm(JavaVMInitArgs* args, bool* canTryAgain) {
   ...
   * wangyang
   */
  JavaThread* main_thread = new JavaThread(); // --> 这里 创建  一个 java线程 ，也就是main函数 线程 wangyang important
   ...
   *
   */
  jint status = init_globals(); //加载全局变量 wangyang important 这里加载stub
  if (status != JNI_OK) {
    delete main_thread;
    *canTryAgain = false; // don't let caller call JNI_CreateJavaVM again
    return status;
   ...
   *
   * wangyang 重要
   * 计算出  SystemDictionary java systemclassloader
   *	在这里 获取 SystemclassLoader 为 appClassLoader
   */
