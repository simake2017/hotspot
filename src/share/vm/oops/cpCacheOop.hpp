...
// field_index = index into field information in holder instanceKlass //--> 在 instanceKlass中 的位置
...
// _f1      = field holder //--> 字段持有者 所以就是 instanceKlass 类型
// _f2      = field offset in words //--> 字段的相关偏移   
...
class ConstantPoolCacheEntry VALUE_OBJ_CLASS_SPEC {
 ...
 private:
  volatile intx     _indices;  // constant pool index & rewrite bytecodes //--> 常量池 索引位置
  volatile oop      _f1;       // entry specific oop field  //--> 
  volatile intx     _f2;       // entry specific int/oop field //--> 字段的偏移
