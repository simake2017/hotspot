...
class Klass : public Klass_vtbl {
 ...
 protected:
  ...
  //
  // For instances, layout helper is a positive number, the instance size. --> 对实例而言 是 正数 表名实例大小的的数值
  // This size is already passed through align_object_size and scaled to bytes.
  // The low order bit is set if instances of this class cannot be
  // allocated using the fastpath.
  //
  // For arrays, layout helper is a negative number, containing four  //--> 对数组而言是负数
  ...
  Symbol*     _name; //wangyang 用于表示 class Name 
 ...
 protected:
  ...
  oop       _java_mirror; //wangyang --> 这里会获取一个 镜像类 
  // Superclass
  klassOop  _super;
