...
// A Symbol is a canonicalized string. -->是一个规范化的 字符串
// All Symbols reside in global SymbolTable and are reference counted. 所有Symbol属于 全局 SymbolTable 并且计数
...
// collector can free the Symbol and remove it from the SymbolTable. //-->当一个符号引用 计数 到 0，gc可以释放和 从 SymbolTable移除
//
...
class Symbol : public CHeapObj {
 ...
 private:
  volatile int   _refcount; //这个Symbol 的 引用 计数
  int            _identity_hash; //哈希值
  unsigned short _length; // number of UTF8 characters in the symbol
  jbyte _body[1]; //--> 从body位置开始 就是 对应的name 值 ,是 字符串 的 utf8 字节 的编码 值

  enum {
    // max_symbol_length is constrained by type of _length
    max_symbol_length = (1 << 16) -1 //--> 最大长度
  };
