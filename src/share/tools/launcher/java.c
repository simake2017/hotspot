 ...
 * wangyang
 * argc 是参数个数
 * argv 是二级指针  ，指针通常都用 char类型（一个字节） ，argv[0] 其实就是 一个 指针，指向 一维数组
 */
int
main(int argc, char ** argv)
{
    ...
    if (_launcher_debug)
      start = CounterGet();
    if (!LoadJavaVM(jvmpath, &ifn)) { //这里 wangyang
      exit(6);
    }
    if (_launcher_debug) {
      end   = CounterGet();
...
JavaMain(void * _args)
{
    ...
    /**
     * 这里只是声明了 一个 指针类型 指向JavaVM类型 然后 赋初值为0
     *
     */
    JavaVM *vm = 0;
    /**
     * 这里也是 赋初值为 0
     */
    JNIEnv *env = 0; //这个类是 c代码类，所以 这里的 env 指的是 JNINativeInterface_
    jstring mainClassName;
    jclass mainClass; //对象类型 指针
    jmethodID mainID;
    ...
    if (_launcher_debug)
        start = CounterGet();
    /*
     * 这里执行环境创建 vm本身是一个指针类型 这里又进行了 取地址操作
     * 实际上就是二级指针 下面两个参数也是一样的
     * important
     * wangyang
     */
     ...
     */
    if (jarfile != 0) {
        mainClassName = GetMainClassName(env, jarfile); // main 线程的 class name
        if ((*env)->ExceptionOccurred(env)) {
            ReportExceptionDescription(env);
            goto leave;
        }
        ...
        classname = (char *)(*env)->GetStringUTFChars(env, mainClassName, 0);  //获取 className
        if (classname == NULL) {
            ReportExceptionDescription(env);
            goto leave;
        }
        ...
        /*
         * 这里是 主类  加载
         * 根据 classname 与 env 加载 主类 class
         */
