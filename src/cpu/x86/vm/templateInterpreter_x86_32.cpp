...
/*
 * wangyang
 * 生成     normal entry_point 进入点
 * 使用类外定义的方式 ，定义 generate_normal_entry函数，表示zerolocals的 正常 进入 点
 */
 ...
 * wangyang
 *
 * 这里的 函数的 参数 是枚举类型，所以可以直接使用 zerolocals这样的类型
 *
 * 可以看到 下面 entry_point有很多中，对于不同的方法，对应的entry_point是不同的
 */
 ...
 * wangyang
 * 具体实例化 stub 例程
 * ，初始化 InterpreterGenerator的同时，初始化他的父类
 */
